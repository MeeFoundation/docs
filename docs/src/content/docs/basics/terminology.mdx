...
A web service that manages the user's data in mulitple distributed Self-managed or Co-managed Personal Datastores (CDSes).

If the user is also the holder of a *local smartwallet,* the PDS will synchronize its personal data state with it. 

A PDS co-manages the personal data in a app provider's Co-managed Personal Datastore (CDS_. We say co-manages because on a fine-grained attribute by attribute level access rights may be controlled by the PDS user, the provider, or a combination of the two. 
...
A single PDS may be linked to multiple provider's web *apps*. The personal data that has been shared with (or generated within) the provider's web *app* is stored and managed within that *app*.
...
A *subject* with which the holder interacts. An *RP* may be another *holder* or a *provider*.
