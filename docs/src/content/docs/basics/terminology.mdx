...
- Has features that offer privacy protection (e.g. implementing https://globalprivacycontrol.org).
- The state of a *local smartwallet* may be replicated (i.e. copied and synchronized) across multiple *physical* replicas with each replica running as its own application. For example a *holder* could have an iOS *smartwallet* app whose state it replicated bidirectionally with their MacOS *smartwallet* app. This affords the *holder* the convenience of being able to pick up and use any of their *smartwallet* apps knowing that each has the same state (i.e. the same set of connections, etc.).
...
#### Self-hosted Smartwallet

A *local smartwallet* that runs on a local machine that is always available. 
