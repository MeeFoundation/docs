...
- A portion of the state of a logical wallet may be replicated across multiple, *partial* physical wallets. Each *partial* wallet may contain as few as one *context* (along with its own *Self* replica) or all of the contexts of a logical wallet. Typically a partial wallet contains only one context.
