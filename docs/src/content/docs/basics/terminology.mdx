...
An independent entity that [relying parties](#relying-party) (service providers in that case) create or join and to which they provide information available for them in [Mee Data Network](#mee-data-network). It combines the information it receives from its members, and uses this data to respond to [Personal Ad Profile](#personal-ad-profile) matching requests from [Ad Profile Service](#ad-profile-service).

[Ad Profile Agent](#ad-profile-agent) itself doesn't persist any actual users' data, but only combines profile scoring results, computed by its members. In performance critical scenarios it may holds users' data temporary, only while scoring profiles.
...
A website, web service, mobile, tablet, laptop or desktop [app](#app) (or an another user's [identity-agent](#identity-agent) that communicates with this user's [identity-agent](#identity-agent)).
...
A representation of the user's relationship with an [app](#app). The user's [identity-agent](#identity-agent) shows a list of [connections](#connection) each one of which represents a data sharing communications channel with a different [app](#app). Information flows periodically or continuously. It flows from an [app](#app) to the [identity agent](#identity-agent), in the reverse direction, or in both directions. It flows using one or more data sharing protocols. 

A [connection's](#connection) represented within an [identity agent](#identity-agent) by one or more [contexts](#contexts)--one for each data sharing protocol used by the [connection(#connection)]. Information (specifically, [claim](#claim) values) shared using a [context's](#context) protocol may or may not be stored (permanently or temporarily) by the [identity agent](#identity-agent) depending on the protocol used, and whether the context is [virtual contexts](#virtual-context) or [physical contexts](#physical-context).
...
A container that represents a protocol-specific aspect of a [connection](#connection). The [context](#context) contains entities that represent the user [subject](#subject) and may, in social contexts, other [subjects](#subject). 

### Persistent Context

A [context](#context) whose contained [subject](#subject) entities are identified by an identifier and a set of [claims](#claim) with values that are stored permanently. 

### Transient Content

A [context](#context) whose contained [subject](#subject) entities are identified by an identifier and a set of [claims](#claim) with values that are stored only transiently. 

### Meta Context

A [context](#context) whose contained [subject](#subject) entities are identified by an identifier and a set of [claims](#claim) without values.  [Claim](#claim) values, when needed, may be fetched from the [app](#app) using the [context's](#context) protocol.

## Identity Agent

An [identity-agent](#identity-agent) is a software program that represents the user on the internet. It manages [personal information](#personal-information) about the user in their interactions with [apps](#app).  Depending on the whether the agent is a [virtual agent](#virtual-agent) or a  [local agent](#local-agent), this information may be stored within an [app](#app), and/or on-device, and/or by a third-party storage provider service.
...
A [subject](#subject) with which the user interacts using an *identity agent*. An [relying party](#relying-party) may be another user (of an [identity agent](#identity-agent)) or a service provider.
