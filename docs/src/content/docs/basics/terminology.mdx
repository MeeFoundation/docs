...
A *digital wallet* is comprised of two components: an agent and a Self-managed Personal Datastore (SPD). The state of a *local wallet* is held in the SPD. The agent is responsible for interacting with the outside world (e.g. issuers and *relying parties*). The agent is an active component that works on behalf the user and manages the contents of the SPD. 
