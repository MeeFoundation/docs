...
A *digital wallet* that is partially centralized on a webservice and partially embedded within *apps*. The holder's personal data that has been shared with (or generated within) each first-party app remains within that app and is not stored in the center which acts as a coordinator of the multiple fragments embedded within apps.

##### Self-hosted wallet

A *digital walle*t that runs on a computer that the person hosts themselves and that is always available on the internet. 
...
Smartwallets may be local wallets (called **Local Smartwallets**), virtual wallets (called **Virtual Smartwallets**) or self-hosted wallets (called **Self-hosted Virtual Wallets**)
