...
### MDN & Virtual Agent Project

The Mee Data Network (MDN) and virtual agent project will allow the user to control their own personal data held by compatible internet service providers by using either a *virtual* identity agent hosted by the Foundation (planned for itsmee(dot)org), or the Mee Smartwallet app. The company logos in the following diagram below are purely hypothetical/aspirational:

![MDN](../images/MDN.png)

The MDN is a set of MDN-compatible service provider’s mobile and web apps that store and manage personal data under the terms of a privacy-respecting MDN license agreement. These apps implement MDN communications protocols that enable this information to be controlled by identity agents working on behalf of the user.
...
Mee is a work in progress; the future directions and milestones are described in our [Roadmap](/users/contributing/roadmap). The [Releases](/users/releases) page lists the deliverables available right now. Here is some of the [Terminology](/users/basics/terminology) we use.
